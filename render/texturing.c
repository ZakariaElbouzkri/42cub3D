...
/*   Updated: 2023/11/22 04:15:01 by zel-bouz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
uint32_t	my_pixel_put(mlx_texture_t *texture, int x, int y)
{
	...
	rgba.red = texture->pixels[((y * size + x * texture->bytes_per_pixel) + 0)];
	rgba.green = texture->pixels[((y * size + x * texture->bytes_per_pixel) + 1)];
	rgba.blue = texture->pixels[((y * size + x * texture->bytes_per_pixel) + 2)];
	rgba.alpha = texture->pixels[((y * size + x * texture->bytes_per_pixel) + 3)];
